...
gulp.task("build:jekyll", cb =>
  cp
    .spawn(
      "jekyll",
      ["build", "--config", "./_config.yml", "--dest", EXPORT_PATH],
      { stdio: "inherit" }
...
gulp.task("serve", () =>
  cp
    .spawn(
      "bundle",
      ["exec", "jekyll", "serve", "--incremental", "--dest", EXPORT_PATH],
      { stdio: "inherit" }
    ) // Adding incremental reduces build time.
    .on("error", error => gutil.log(gutil.colors.red(error.message)))
);
...
gulp.task("prefix", () =>
  gulp
    .src(`${EXPORT_PATH}/**/*.html`)
    .pipe(
      prefix("/nhs.portal.web", [
